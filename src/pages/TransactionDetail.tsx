          ...
          <nav className="flex gap-4 border-t pt-4">
            {tabs.map((tab) => (
              <button
                ...
                className={`text-subheadline-emphasized pb-2 border-b-2 transition-colors ${
                  activeTab === tab.id
      ...
      {events.map((event) => (
        <div key={event.id} className="border-l-2 border-blue-600 pl-4 pb-4">
          <div className="flex justify-between items-start">
            <div>
              <h3 className="text-body-emphasized">{event.title}</h3>
              <p className="text-subheadline text-secondary mt-1">{new Date(event.date).toLocaleString()}</p>
            </div>
    ...
    <div className="space-y-4">
      ...
      <div className="mt-4 pt-4 border-t">
        <textarea
          ...
          className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-body"
          rows={3}
        />
        <button className="mt-2 px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 text-subheadline-emphasized">
          Send Message
        </button>
      </div>
    </div>
